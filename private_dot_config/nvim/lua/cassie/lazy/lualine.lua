...
local function git_statusline()
	...
	local upstream = vim.fn.systemlist("git rev-parse --abbrev-ref --symbolic-full-name @{upstream} 2>/dev/null")[1]
		or ""
	...
	if upstream ~= "" then
		...
		if counts and counts ~= "" then
			...
			if a and tonumber(a) > 0 then
				ahead = colored("↑" .. a, "GitStatusAhead")
			end
			if b and tonumber(b) > 0 then
				behind = colored("↓" .. b, "GitStatusBehind")
			end
		end
	end
	...
	local ab = {}
	if ahead ~= "" then
		table.insert(ab, ahead)
	end
	if behind ~= "" then
		table.insert(ab, behind)
	end
	if #ab > 0 then
	...
	local changes = {}
	if added > 0 then
		table.insert(changes, colored("+" .. added, "GitStatusAdded"))
	end
	if modified > 0 then
		table.insert(changes, colored("~" .. modified, "GitStatusModified"))
	end
	if removed > 0 then
		table.insert(changes, colored("-" .. removed, "GitStatusRemoved"))
	end
	if #changes > 0 then
